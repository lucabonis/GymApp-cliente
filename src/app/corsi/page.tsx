...
const CACHE_SESSIONI = 'cache_sessioni_v1';
const CACHE_BOOKINGS = 'cache_bookings_v1';
const QUEUE_KEY = 'queue_prenotazioni_v1';
...
export default function CorsiPage() {
  ...
  useEffect(() => {
    ...
    try {
      const cs = localStorage.getItem(CACHE_SESSIONI);
      const cb = localStorage.getItem(CACHE_BOOKINGS);
      if (cs) setSessioni(JSON.parse(cs));
      if (cb) {
        const bk = JSON.parse(cb);
        setBookings(bk.map((b: any) => b.session_id));
        setPrenotazioni(bk);
      }
      if (cs || cb) setLoading(false);
    ...
    const onOnline = () => { setOffline(false); syncQueue().then(() => loadData()); };
  ...
  async function loadData() {
    ...
    const [{ data: sess }, { data: bk }] = await Promise.all([
      ...
      supabase.from('course_bookings').select('session_id,id,status,booked_at,course_sessions(start_time,courses(name,color))').eq('client_id', uid).eq('status', 'confirmed'),
    ]);
    if (!sess && !bk) return; // offline
    if (sess) { setSessioni(sess); try { localStorage.setItem(CACHE_SESSIONI, JSON.stringify(sess)); } catch {} }
    if (bk) {
      setBookings(bk.map((b: any) => b.session_id));
      setPrenotazioni(bk);
      try { localStorage.setItem(CACHE_BOOKINGS, JSON.stringify(bk)); } catch {}
    }
    setLoading(false);
  }
  ...
  async function syncQueue() {
    try {
      ...
      for (const action of queue) {
        if (action.type === 'book') {
          await supabase.from('course_bookings').upsert({ session_id: action.sessionId, client_id: action.userId, gym_id: GYM_ID, status: 'confirmed', booked_at: new Date().toISOString() }, { onConflict: 'session_id,client_id' });
        } else {
          await supabase.from('course_bookings').update({ status: 'cancelled', cancelled_at: new Date().toISOString() }).eq('session_id', action.sessionId).eq('client_id', action.userId);
        }
      }
  ...
  async function bookSession(sid: string) {
    ...
    if (!navigator.onLine) {
      try {
        const queue = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
        queue.push({ type: 'book', sessionId: sid, userId });
  ...
  async function cancelBooking(sid: string) {
    // Ottimistico
    setBookings(prev => prev.filter(b => b !== sid));
    setPrenotazioni(prev => prev.filter((b: any) => b.session_id !== sid));
    if (!navigator.onLine) {
      try {
        const queue = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
        queue.push({ type: 'cancel', sessionId: sid, userId });
        localStorage.setItem(QUEUE_KEY, JSON.stringify(queue));
        setPendingSync(queue.length);
      } catch {}
      return;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '16px 20px', paddingTop: 'max(env(safe-area-inset-top,16px),16px)', borderBottom: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {offline && <div style={{ fontSize: 11, color: '#f59e0b', fontWeight: 600 }}>📵 Offline{pendingSync > 0 ? ` · ${pendingSync} in coda` : ''}</div>}
          {!offline && pendingSync > 0 && <div style={{ fontSize: 11, color: '#22c55e', fontWeight: 600 }}>🔄 Sincronizzando...</div>}
        </div>
