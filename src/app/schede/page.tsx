...
const CACHE_ASSIGNMENTS = 'cache_assignments_v1';
const CACHE_LOGS = 'cache_logs_v1';
const QUEUE_LOGS = 'GA_pending_logs';
...
export default function SchedePage() {
  ...
  useEffect(() => {
    ...
    try {
      const ca = localStorage.getItem(CACHE_ASSIGNMENTS);
      const cl = localStorage.getItem(CACHE_LOGS);
      if (ca) setAssignments(JSON.parse(ca));
      if (cl) setLogs(JSON.parse(cl));
      if (ca || cl) setLoading(false);
    } catch {}
    ...
    const onOnline = () => {
      setOffline(false);
      // Sync automatica: invia log salvati offline
      syncPendingLogs().then(() => loadList());
    };
  ...
  async function loadList() {
    ...
    if (wlogs) { setLogs(wlogs); try { localStorage.setItem(CACHE_LOGS, JSON.stringify(wlogs)); } catch {} }
    setLoading(false);
  }
  ...
  async function syncPendingLogs() {
    try {
      const queue = JSON.parse(localStorage.getItem(QUEUE_LOGS) || '[]');
      if (queue.length === 0) return;
      for (const entry of queue) {
        await supabase.from('workout_logs').insert(entry);
      }
      localStorage.removeItem(QUEUE_LOGS);
  ...
  async function doFinish() {
    ...
    const entry = { client_id: userId, gym_id: GYM_ID, template_id: templateId, day_id: savedDayId, template_name: schedaName, day_name: dayName, duration_seconds: duration, logs: exLogs, completed_at: new Date().toISOString() };

    if (!navigator.onLine) {
      // Offline: metti in coda locale
      try {
        const queue = JSON.parse(localStorage.getItem(QUEUE_LOGS) || '[]');
        queue.push(entry);
        localStorage.setItem(QUEUE_LOGS, JSON.stringify(queue));
        // Aggiorna cache logs localmente
        const cached = JSON.parse(localStorage.getItem(CACHE_LOGS) || '[]');
        cached.unshift({ ...entry, id: `local_${Date.now()}` });
        localStorage.setItem(CACHE_LOGS, JSON.stringify(cached));
        setLogs(cached);
      } catch {}
      setPendingSync(true);
    } else {
      await supabase.from('workout_logs').insert(entry);
      // Sync eventuali log in coda
  ...
  async function deleteLog(logId: string) {
    ...
    try { localStorage.setItem(CACHE_LOGS, JSON.stringify(logs.filter(l => l.id !== logId))); } catch {}
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '16px 20px', paddingTop: 'max(env(safe-area-inset-top,16px),16px)', borderBottom: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {offline && <div style={{ fontSize: 11, color: '#f59e0b', fontWeight: 600 }}>📵 Offline{pendingSync ? ' · sync in attesa' : ''}</div>}
          {!offline && pendingSync && <div style={{ fontSize: 11, color: '#22c55e', fontWeight: 600 }}>🔄 Sincronizzando...</div>}
        </div>
