...
const CACHE_KEY = 'cache_profilo_v2';
...
export default function ProfiloPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cached) {
        const { profile: p, sub: s, payments: pay } = JSON.parse(cached);
        setProfile(p); setSub(s); setPayments(pay); setLoading(false);
      }
    } catch {}
  ...
  async function loadData() {
    ...
    try { localStorage.setItem(CACHE_KEY, JSON.stringify({ profile: prof, sub: subscription || null, payments: allPayments })); } catch {}
    setLoading(false);
  }
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 18, padding: 20 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {profile?.phone && <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>📞 {profile.phone}</div>}
            </div>
          </div>
                ...
                <div style={{ borderTop: '1px solid var(--border)' }}>
                  {payments.map(p => (
                    <div key={p.id} style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                      ...
                      {p.price != null && <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)', flexShrink: 0 }}>€{Number(p.price).toFixed(2)}</div>}
                    </div>
                  ))}
                </div>
              )}
